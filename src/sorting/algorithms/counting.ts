...
export function* countingSort(input: number[]): Generator<Step> {
    const arr = [...input];
    ...
    for (let index = 0; index < arr.length; index++) {
        ...
        yield {
            ...
            workspace: buildWorkspace(
                `Counting value ${value} at input index ${index}`,
                count,
                output,
                filled,
                value
    ...
    for (let value = 1; value < count.length; value++) {
        ...
        yield {
            ...
            memoryUnit: "items",
            workspace: buildWorkspace(
                `Building prefix sums at count index ${value}`,
                count,
                output,
                filled,
                value
    ...
    for (let index = arr.length - 1; index >= 0; index--) {
        ...
        const display = arr.map((currentValue, fillIndex) =>
            filled[fillIndex] ? output[fillIndex] : currentValue
        );
        ...
        yield {
            ...
            memoryUnit: "items",
            workspace: buildWorkspace(
                `Writing value ${value} to output index ${outputIndex}`,
                count,
                output,
                filled,
                value,
    ...
    yield {
        ...
        memoryUnit: "items",
        workspace: buildWorkspace(
            "Final sorted output reconstructed from memory",
            count,
            output,
            filled
        )
