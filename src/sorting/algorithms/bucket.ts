...
export function* bucketSort(input: number[]): Generator<Step> {
    const arr = [...input];
    ...
    for (let bucketIndex = 0; bucketIndex < buckets.length; bucketIndex++) {
        ...
        yield {
            ...
            workspace: createWorkspace(
                buckets,
                output,
                `Sort the values inside bucket ${bucketIndex}`,
                bucketIndex
            )
        };
        ...
        for (const value of bucket) {
            ...
            yield {
                ...
                workspace: createWorkspace(
                    buckets,
                    output,
                    `Write ${value} from bucket ${bucketIndex} into output position ${outputIndex}`,
                    bucketIndex
                )
            };
    ...
    yield {
        ...
        workspace: createWorkspace(
            buckets,
            output,
            "All buckets have been merged into the final sorted output"
        )
    };
}
