...
function createWorkspace(
    ...
    detail: string,
    depthLimit: number,
    focusValues: Array<number | null> = [null],
    activeRangeIndices: number[] = [],
    fullArrayHighlights: number[] = []
): WorkspaceState {
...
export function* introSort(input: number[]): Generator<Step> {
    const arr = [...input];
    ...
    function memoryForDepth(depthLimit: number) {
        return Math.max(maxDepth - depthLimit, 0);
    }

    function* insertionSortRange(low: number, high: number, depthLimit: number): Generator<Step> {
        for (let index = low + 1; index <= high; index++) {
            ...
            while (position >= low && arr[position] > key) {
                yield {
                    ...
                    workspace: createWorkspace(
                        ...
                        low,
                        high,
                        `Use Insertion Sort on the small range ${low} to ${high}`,
                        depthLimit,
                        [key],
                        [position, position + 1],
                        [position, position + 1]
                ...
                yield {
                    ...
                    workspace: createWorkspace(
                        ...
                        low,
                        high,
                        `Shift ${arr[position + 1]} right while inserting ${key}`,
                        depthLimit,
                        [key],
                        [position, position + 1],
                        [position, position + 1]
            ...
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    high,
                    `Place ${key} into its sorted spot inside the small range`,
                    depthLimit,
                    [key],
                    [position + 1],
                    [position + 1]
    ...
    function* heapify(low: number, length: number, index: number, depthLimit: number): Generator<Step> {
        let largest = index;
        ...
        if (left < length) {
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    low + length - 1,
                    `Depth limit reached, so Intro Sort is using Heap Sort on this range`,
                    depthLimit,
                    [arr[low + largest], arr[low + left]],
                    [low + largest, low + left],
                    [low + largest, low + left]
                )
            };
        ...
        if (right < length) {
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    low + length - 1,
                    `Continue Heap Sort comparisons inside the fallback range`,
                    depthLimit,
                    [arr[low + largest], arr[low + right]],
                    [low + largest, low + right],
                    [low + largest, low + right]
                )
            };
        ...
        if (largest !== index) {
            ...
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    low + length - 1,
                    `Swap nodes to restore heap order inside the fallback range`,
                    depthLimit,
                    [arr[low + index], arr[low + largest]],
                    [low + index, low + largest],
                    [low + index, low + largest]
                )
            };
    ...
    function* heapSortRange(low: number, high: number, depthLimit: number): Generator<Step> {
        ...
        for (let end = length - 1; end > 0; end--) {
            ...
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    high,
                    `Move the current maximum to the end of the heap-fallback range`,
                    depthLimit,
                    [arr[low + end]],
                    [low, low + end],
                    [low, low + end]
    ...
    function* partition(low: number, high: number, depthLimit: number): Generator<Step, number> {
        ...
        for (let index = low; index < high; index++) {
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    high,
                    `Use Quick Sort partitioning on the active range with pivot ${pivot}`,
                    depthLimit,
                    [pivot],
                    [index, high],
                    [index, high]
            ...
            if (arr[index] < pivot) {
                ...
                yield {
                    ...
                    workspace: createWorkspace(
                        ...
                        low,
                        high,
                        `Move ${arr[storeIndex]} to the left partition before the pivot`,
                        depthLimit,
                        [pivot],
                        [storeIndex, index],
                        [storeIndex, index, high]
                    )
                };
        ...
        yield {
            ...
            workspace: createWorkspace(
                ...
                low,
                high,
                `Place pivot ${pivot} into its final partition position`,
                depthLimit,
                [pivot],
                [storeIndex, high],
                [storeIndex, high]
    ...
    function* sort(low: number, high: number, depthLimit: number): Generator<Step> {
    ...
    yield {
        ...
        workspace: createWorkspace(
            ...
            0,
            arr.length - 1,
            "Quick, heap, and insertion phases are complete",
            0
        )
    };
}
