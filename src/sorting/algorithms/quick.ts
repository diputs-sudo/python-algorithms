...
export function* quickSort(input: number[]): Generator<Step> {
    const arr = [...input];
    ...
    function* partition(low: number, high: number, depth: number): Generator<Step, number> {
        ...
        for (let j = low; j < high; j++) {
            yield {
                ...
                workspace: createWorkspace(
                    ...
                    low,
                    high,
                    `Compare ${arr[j]} against pivot ${pivot} while partitioning`,
                    high,
                    [j, high],
                    [j, high]
                )
            ...
            if (arr[j] < pivot) {
                ...
                yield {
                    ...
                    workspace: createWorkspace(
                        ...
                        low,
                        high,
                        `Move ${arr[i]} into the left partition before the pivot`,
                        high,
                        [i, j, high],
                        [i, j, high]
                    )
    ...
    function* sort(low: number, high: number, depth: number): Generator<Step> {
        if (low < high) {
