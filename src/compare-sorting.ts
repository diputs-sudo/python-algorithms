...
import { Step, WorkspaceState } from "./sorting/visualizer/types.js";
...
type CompareLanguage = "python" | "cpp" | "java" | "c";
...
type AlgorithmInfo = {
    ...
    details: string[];
    codeBaseName?: string;
    requiresNonNegativeIntegers?: boolean;
};
...
const algorithmInfo: Record<AlgorithmKey, AlgorithmInfo> = {
    bitonic_sort: {
        title: "Bitonic Sort",
...
const algorithmOrder: AlgorithmKey[] = [
    "bitonic_sort",
    "bubble_sort",
    "bucket_sort",
    "counting_sort",
...
function createSlot(slotId: "One" | "Two"): CompareSlot {
    const select = document.getElementById(`compareSlot${slotId}`) as HTMLSelectElement;

    return {
        select,
        title: document.getElementById(`compareSlot${slotId}Title`) as HTMLElement,
        summary: document.getElementById(`compareSlot${slotId}Summary`) as HTMLElement,
        tags: document.getElementById(`compareSlot${slotId}Tags`) as HTMLElement,
        details: document.getElementById(`compareSlot${slotId}Details`) as HTMLElement,
        link: document.getElementById(`compareSlot${slotId}Link`) as HTMLAnchorElement,
        visualizerTitle: document.getElementById(`compareVisualizer${slotId}Title`) as HTMLElement,
        codeTitle: document.getElementById(`compareCode${slotId}Title`) as HTMLElement,
        codeDisplay: document.getElementById(`compareCodeDisplay${slotId}`) as HTMLElement,
        languageButtons: Array.from(
            document.querySelectorAll(`.lang-btn[data-slot="${slotId === "One" ? "one" : "two"}"]`)
        ) as HTMLButtonElement[],
        language: "python",
        algorithm: select.value as AlgorithmKey
    };
}

function getCodeBaseName(algorithm: AlgorithmKey, language: CompareLanguage): string {
    if (language === "java") {
...
async function loadCode(slot: CompareSlot) {
    const extensions: Record<CompareLanguage, string> = {
        python: "py",
        cpp: "cpp",
        java: "java",
        c: "c"
...
function renderSlot(slot: CompareSlot) {
    slot.algorithm = slot.select.value as AlgorithmKey;
...
function createGenerator(algorithm: AlgorithmKey, dataset: number[]): Generator<Step> {
    switch (algorithm) {
        case "bitonic_sort":
...
function sanitizeDataset(values: number[], algorithms: AlgorithmKey[]): number[] {
    const requiresNonNegativeIntegers = algorithms.some(
        algorithm => algorithmInfo[algorithm].requiresNonNegativeIntegers
    );
...
function createInitialWorkspace(algorithm: AlgorithmKey, dataset: number[]): WorkspaceState | undefined {
    switch (algorithm) {
        case "bitonic_sort": {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const datasetInput = document.getElementById("compareDatasetInput") as HTMLInputElement;
    const generateBtn = document.getElementById("compareGenerateBtn") as HTMLButtonElement;
    const playBtn = document.getElementById("comparePlayBtn") as HTMLButtonElement;
    const pauseBtn = document.getElementById("comparePauseBtn") as HTMLButtonElement;
    const stepBtn = document.getElementById("compareStepBtn") as HTMLButtonElement;
    const resetBtn = document.getElementById("compareResetBtn") as HTMLButtonElement;
    const speedRange = document.getElementById("compareSpeedRange") as HTMLInputElement;
    ...
    slotOne.algorithm = slotOne.select.value as AlgorithmKey;
    slotTwo.algorithm = slotTwo.select.value as AlgorithmKey;
    ...
    const copyButtons = Array.from(
        document.querySelectorAll(".copy-code-btn")
    ) as HTMLButtonElement[];

    [slotOne, slotTwo].forEach(slot => {
        slot.languageButtons.forEach(button => {
            button.addEventListener("click", () => {
                slot.language = button.dataset.lang as CompareLanguage;
    ...
    datasetInput.addEventListener("change", () => {
        const values = sanitizeDataset(
            datasetInput.value
                .split(",")
                .map(value => Number(value.trim()))
                .filter(value => !Number.isNaN(value)),
            [slotOne.algorithm, slotTwo.algorithm]
        );
