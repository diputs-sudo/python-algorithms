function createWorkspace(buckets, output, exp, detail, activeBucket) {
    return {
...
export function* radixSort(input) {
    ...
    let exp = 1;
    let lastBuckets = Array.from({ length: 10 }, () => []);
    let lastOutput = arr.map(value => value);
    let lastExp = 1;
    while (Math.floor(maxNumber / exp) > 0) {
        ...
        for (let index = 0; index < arr.length; index++) {
            ...
            yield {
                ...
                memoryLabel: "Memory",
                memoryUnit: "items",
                workspace: createWorkspace(buckets, output, exp, `Place ${value} into bucket ${digit}`, digit)
            };
        }
        let writeIndex = 0;
        for (let bucketIndex = 0; bucketIndex < buckets.length; bucketIndex++) {
            ...
            for (const value of bucket) {
                ...
                yield {
                    ...
                    memoryLabel: "Memory",
                    memoryUnit: "items",
                    workspace: createWorkspace(buckets, output, exp, `Collect ${value} from bucket ${bucketIndex} into position ${writeIndex}`, bucketIndex)
                };
                writeIndex += 1;
            }
        }
    ...
    yield {
        ...
        memoryLabel: "Memory",
        memoryUnit: "items",
        workspace: createWorkspace(lastBuckets, lastOutput, lastExp, "All digit passes are complete")
    };
}
